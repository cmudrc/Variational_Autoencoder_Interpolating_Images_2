...
def make_basic_boxes(i: int, j: int, density_increment, image_size) -> tuple:
    ...
    # plt.colorbar()
    # plt.show()
    new = (A, density, number_of_additional_pixels, "Basic_Box")
    return new
...
def make_diagonal_split_boxes(i: int, j: int, density_increment, image_size) -> tuple:
    ...
    # plt.colorbar()
    # plt.show()
    new = (A, density, number_of_additional_pixels, "Diagonal_Box_Split")
    return new
...
def make_horizontal_vertical_split_boxes(i: int, j: int, density_increment, image_size) -> tuple:
    ...
    new = (A, density, number_of_additional_pixels, "Horizontal_Box_Split")
    return new
...
test_row = 65
plt.matshow(box_data[test_row][0], cmap='gray')  # Where index1 is the row, and index2 is the column(which should remain 0)
plt.title(str(box_data[test_row][3]) + "\nPixel Density: " + str(box_data[test_row][1]) + "\nAdditional Pixels: " + str(box_data[test_row][2]))
plt.colorbar()
plt.show()
